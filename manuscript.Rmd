...
example of a way forward in addressing these fundamental concerns. 
...
that it be easy to learn and fit relatively seamlessly into existing
workflow patterns of domain researchers.  This,
...
to go back to the original image. Before others can re-use our image,
it must be made available. Docker provides a convenient way to do this
...
or privately through the Hub after creating a free account.  For instance,
...
image used as a base in the first Dockerfile.  To do so, one simply specifies a
different image in the `FROM` directive of the Dockerfile, followed by the lines
required to add any additional software required.
...
approach [@Leisch_2002; @Peng_2011; @Xie_2013], which seek to embed
