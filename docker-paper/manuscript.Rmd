...
the dependency can be specified using the `:` notation, or ommitted to
...
Consider the following Dockerfile, which 

```
FROM rocker/ropensci:latest
RUN git clone https://github.com/ropensci/RNeXML.git /rnexml
RUN Rscript -e 'devtools::install("/rnexml"); devtools::check("/rnexml")'
```


A Dockerfile can have only a single `FROM` line, while sometimes it may
...
to link it together with other blocks.  

```bash
docker run --name tex -v /usr/local/texlive rocker/texlive
