...
and differencing system, among others (see [docs.docker.com/faq](https://docs.docker.com/faq/#what-does-docker-add-to-just-plain-lxc) for
an excellent overview of what Docker adds to plain LXC). 
...
<!-- Part of the reason is better image distribution-->
...
Dockerfile must have exactly one dependency (one `FROM` line). Note
that a particular version of the dependency can be specified using the
`:` notation (similarly, package managers like `apt` show here can be
directed to install specific versions, if desired. One can in fact be much
more precise by specific cryptographic hash of a particular binary image.
In practice, using hashes in FROM directives may be undesirable as it also
avoids adopting security updates that may have been made to the base image.

One can name and build this Dockerfile by running in the
working directory:
...
`ubuntu:14.04` image used as a base in the first Dockerfile.  To do
...
```
This separates the task fo running the database (first line) from
