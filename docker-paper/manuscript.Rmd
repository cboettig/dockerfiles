...
cloud computing on virtual machines [_e.g._ @Dudley_2010; @Howe_2012]. 
...
environments [_e.g._ @Dudley_2010], many researchers work _locally_, that
...
That will take a while to download the image the first time you run it. A
boot2docker user will then need to determine the ip address assigned
to boot2docker as shown below, while a Linux user can just specify
`http://localhost`.
...
a particular version of the dependency can be specified using the `:` notation.
...
result, it is necessary to package the appropriate typesetting libaries
(_e.g._ \LaTeX) along with the code libaries such that the document executes
...
[@Stodden_2010]. Thus, lowering these barrier to re-use through
